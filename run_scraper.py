...
if __name__ == "__main__":
    ...
    parser.add_argument("--delete_headlines", type=bool)

    args = parser.parse_args()

    delete_headlines = args.delete_headlines
    ...
    for site in sites:
        if site in ["Newsmax", "MSNBC"]:
            if delete_headlines:
                scraper.delete_headlines(site, year, month)
            else:
                scraper.write_headlines(site, year, month)
        else:
            if day:
                if delete_headlines:
                    scraper.delete_headlines(site, year, month, day)
                else:
                    scraper.write_headlines(site, year, month, day)
            else:
                for i in range(1, days_in_month + 1):
                    if delete_headlines:
                        scraper.delete_headlines(site, year, month, i)
                    else:
                        scraper.write_headlines(site, year, month, i)